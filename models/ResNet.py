...
from models.layers import Activation,Add
...
class ResNet_vd(nn.Module):
    ...
    def __init__(self,
        ...
        self.classifier=nn.Sequential(
            nn.Linear(self.num_filters[-1],num_classes)
        )
    ...
    def forward(self, inputs):
        ...
        vec=self.adpool(feat4)
        out=self.classifier(torch.nn.Flatten()(vec))
        return out
...
def ResNet18_vd(num_classes,**args):
    model = ResNet_vd(num_classes,layers=18, **args)
    return model


def ResNet34_vd(num_classes,**args):
    model = ResNet_vd(num_classes,layers=34, **args)
    return model



def ResNet50_vd(num_classes,**args):
    model = ResNet_vd(num_classes,layers=50, **args)
    return model



def ResNet101_vd(num_classes,**args):
    model = ResNet_vd(num_classes,layers=101, **args)
    return model
